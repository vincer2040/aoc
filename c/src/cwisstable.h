...
static inline CWISS_ControlByte* CWISS_EmptyGroup() {
    ...
    // write to it, because it is only used for empty tables.
...
CWISS_FindFirstNonFull(const CWISS_ControlByte* ctrl, size_t hash,
    ...
    while (true) {
        ...
        if (mask.mask) {
            ...
            // In debug build we will randomly insert in either the front or
            // back of the group.
            // TODO(kfm,sbenza): revisit after we do unconditional mixing
            if (!CWISS_IsSmall(capacity) &&
                CWISS_ShouldInsertBackwards(hash, ctrl)) {
                return (CWISS_FindInfo){
                    CWISS_ProbeSeq_offset(&seq,
                                          CWISS_BitMask_HighestBitSet(&mask)),
                    seq.index_};
            }
#endif
            return (CWISS_FindInfo){
...
static uint64_t CWISS_AbslHash_LowLevelHash(const void* data, size_t len,
    ...
    while (len > 16) {
        ...
        current_state =
            CWISS_AbslHash_LowLevelMix(a ^ salt[1], b ^ current_state);
    ...
    if (len > 8) {
        ...
        // will overlap in the middle if we are working with less than the full
        // 16 bytes.
        a = CWISS_Load64(ptr);
...
static inline void CWISS_AbslHash_Write(CWISS_AbslHash_State* state,
    ...
    while (len >= CWISS_AbslHash_kPiecewiseChunkSize) {
        CWISS_AbslHash_Mix(
            state,
            CWISS_AbslHash_Hash64(val8, CWISS_AbslHash_kPiecewiseChunkSize));
...
typedef struct {
    ...
    ///
    /// This function is actually not symmetric: the first argument will always
    /// be the value being searched for, and the second will be a pointer to the
    /// candidate entry. In particular, this means they can be different types:
    /// in C++ parlance, `needle` could be a `std::string_view`, while
    /// `candidate` could be a `std::string`.
...
typedef struct {
    /// Allocates memory.
    ///
    /// This function must never fail and never return null, unlike `malloc`.
    /// This function does not need to tolerate zero sized allocations.
    void* (*alloc)(size_t size, size_t align);

    /// Deallocates memory allocated by `alloc`.
    ///
    /// This function is passed the same size/alignment as was passed to
    /// `alloc`, allowing for sized-delete optimizations.
...
typedef struct {
    ...
    ///
    /// This function does not initialize the value *in* the slot; it simply
    /// sets up the slot so that a value can be `memcpy`'d or otherwise emplaced
    /// into the slot.
    void (*init)(void* slot);
...
#define CWISS_DECLARE_NODE_FUNCTIONS_(kPolicy_, Type_, ...)                    \
    ...
    static inline void kPolicy_##_NodeSlotInit(void* slot) {                   \
        void* node =                                                           \
            CWISS_EXTRACT(alloc_alloc, CWISS_DefaultMalloc,                    \
                          __VA_ARGS__)(sizeof(Type_), alignof(Type_));         \
...
typedef struct {
    /// The control bytes (and, also, a pointer to the base of the backing
    /// array).
    ///
...
static inline void CWISS_RawTable_EraseMetaOnly(const CWISS_Policy* policy,
    ...
    const size_t index_before =
        (index - CWISS_Group_kWidth) & it.set_->capacity_;
...
static inline void CWISS_RawTable_Resize(const CWISS_Policy* policy,
    ...
    if (old_capacity) {
        ...
        policy->alloc->free(old_ctrl,
                            CWISS_AllocSize(old_capacity, policy->slot->size,
                                            policy->slot->align),
                            policy->slot->align);
    }
...
static void CWISS_RawTable_DropDeletesWithoutResize(const CWISS_Policy* policy,
    ...
    size_t total_probe_length = 0;
    // Unfortunately because we do not know this size statically, we need to
    // take a trip to the allocator. Alternatively we could use a variable
    // length alloca...
...
#define CWISS_ProbeIndex(pos_)                                                 \
    (((pos_ - probe_offset) & self->capacity_) / CWISS_Group_kWidth)
        ...
        if (CWISS_IsEmpty(self->ctrl_[new_i])) {
            ...
            CWISS_SetCtrl(i, CWISS_kEmpty, self->capacity_, self->ctrl_,
                          self->slots_, policy->slot->size);
        } else {
...
CWISS_RawTable_rehash_and_grow_if_necessary(const CWISS_Policy* policy,
    ...
    if (self->capacity_ == 0) {
        ...
        // long as we are reclaiming at least 1/8 the capacity without doing
        // more than 2X the work.  (Where "work" is defined to be size() for
        // rehashing or rehashing in place, and 1 for an insert or erase.)  But
        // rehashing in place is faster per operation than inserting or even
        // doubling the size of the table, so we actually afford to reclaim even
        // less space from a resize-in-place.  The decision is to rehash in
        // place if we can reclaim at about 1/8th of the usable capacity
        // (specifically 3/28 of the capacity) which means that the total cost
        // of rehashing will be a small fraction of the total work.
        //
        // Here is output of an experiment using the BM_CacheInSteadyState
        // benchmark running the old case (where we rehash-in-place only if we
        // can reclaim at least 7/16*capacity_) vs. this code (which rehashes in
        // place if we can recover 3/32*capacity_).
        //
        // Note that although in the worst-case number of rehashes jumped up
        // from 15 to 190, but the number of operations per second is almost the
        // same.
        //
...
/// key.
static inline void
CWISS_RawTable_PrefetchHeapBlock(const CWISS_Policy* policy,
                                 const CWISS_RawTable* self) {
...
static inline CWISS_RawIter CWISS_RawTable_find_hinted(
    ...
    while (true) {
        ...
        uint32_t i;
        while (CWISS_BitMask_next(&match, &i)) {
            char* slot = self->slots_ +
                         CWISS_ProbeSeq_offset(&seq, i) * policy->slot->size;
            if (CWISS_LIKELY(key_policy->eq(key, policy->slot->get(slot))))
...
#define CWISS_DECLARE_LOOKUP_NAMED(HashSet_, LookupName_, Key_)                \
    ...
    static inline HashSet_##_Insert                                            \
        ...
        CWISS_Insert ret = CWISS_RawTable_deferred_insert(                     \
            HashSet_##_policy(), &HashSet_##_##LookupName_##_kPolicy,          \
            &self->set_, key);                                                 \
...
#define CWISS_DECLARE_COMMON_(HashSet_, Type_, Key_, kPolicy_)                 \
    ...
    static inline HashSet_##_CIter HashSet_##_citer(const HashSet_* self) {    \
        return (HashSet_##_CIter){                                             \
            CWISS_RawTable_citer(&kPolicy_, &self->set_)};                     \
    }                                                                          \
    ...
    static inline bool HashSet_##_contains(const HashSet_* self,               \
                                           const Key_* key) {                  \
        return CWISS_RawTable_contains(&kPolicy_, kPolicy_.key, &self->set_,   \
                                       key);                                   \
    }                                                                          \
                                                                               \
    ...
    static inline bool HashSet_##_erase(HashSet_* self, const Key_* key) {     \
        return CWISS_RawTable_erase(&kPolicy_, kPolicy_.key, &self->set_,      \
                                    key);                                      \
    }                                                                          \
                                                                               \
    CWISS_END                                                                  \
